...
| `borders.json` | 266 KB (82 KB gz) | Country borders, same source. The three.js vector-fill look, and the MapLibre **Countries** chip. |
...
| `atlas-countries.json` | 9 KB (3 KB gz) | 177 country label anchors: name, `[lon, lat]`, min zoom, rank. **MapLibre Countries chip only.** |
| `atlas-cities.json` | 18 KB (6 KB gz) | 243 city label anchors, plus country and a capital flag. **MapLibre Cities chip only.** |
...
| `trails/*.json` | 46 KB total (11 KB gz), 0.5–2 KB gz each | Ten recorded routes, one file per route, as a bare `[[lon, lat], …]` array. Fetched only for the route someone picks. **MapLibre `terrain` mode only.** Generated by `scripts/build-trails.mjs`. |
...
`node scripts/build-trails.mjs [source-dir]`, default `~/Downloads/gpx`, laid out as `hike/` and
`run/`. Nested folders are walked. It writes one file per route here **and** the manifest at
`src/data/trails.json`, which is imported at build time rather than fetched — that split is why
only geometry crosses the network.

Two things it does that are worth not undoing:

- **The figures and the geometry come from different tracks.** Distance, ascent, elevation range
  and point count are measured on the FULL recording; only the drawn line is simplified, at 4 m
  with Douglas–Peucker. Measuring the simplified line instead would quietly shorten every distance
  on the page by a few percent, which is the classic way this goes wrong. 124,219 points become
  2,341 and every printed number is still the watch's.
- **The date is shifted by longitude before it is formatted.** GPX times are UTC and three of the
  ten start before 04:00 local, so printed as UTC they would be dated the previous day. There is no
  timezone in a GPX file. The estimate is an hour out for Sichuan and cannot move any of these
  dates; a recording that starts near local midnight is the case to distrust.

A cross-check that the pipeline is honest: dropping `Lambak_Johor_Malaysia_Morning_Hike.gpx` onto
the frame, which parses in the browser and shares no code with the build script, reads back
5.4 km and 63–518 m — identical to the built-in route. Ascent differs by 2 m (531 against 529)
because the build script drops consecutive duplicate fixes before summing and the browser parser
does not.
...
anonymous dots and needs nothing else, while the MapLibre atlas layers have to be able to say "Bangkok".
...
- `trails/*.json`: the author's **own** GPS recordings, exported from Strava as GPX and
  simplified by `scripts/build-trails.mjs`. Not third-party data, and the only files here
  that are not Natural Earth or NASA. The GPX originals are not in the repo — the script
  reads them from a folder outside it, `~/Downloads/gpx` by default, and 29 MB of
  two-second samples with a heart rate on every point does not belong in git.
...
at runtime — the routes included, which is why picking one costs a request to this site and to
nowhere else.
